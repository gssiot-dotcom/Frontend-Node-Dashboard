...
import { useWorkerBuildingNodesPage } from '../hooks/useWorkerQueries'
...
export default function VerticalNodes() {
	...
	const { data, isLoading, isError } = useWorkerBuildingNodesPage(
		buildingId,
		nodeType,
	)
		...
		setNodesWithUi(prev =>
			prev.map(node => {
				...
				const _alertLevel = sensorData.status
					...
					: mapTiltToUiState(x, y, isOnline, node.status).alertLevel // fallback
